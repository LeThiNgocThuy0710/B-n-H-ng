...
if ($seller_id && $product_id) {
    ...
    $chat_partner_query = "SELECT * FROM nguoidung WHERE MaNguoiDung = ?";
$stmt = $conn->prepare($chat_partner_query);
$stmt->bind_param("i", $seller_id);
$stmt->execute();
$chat_partner = $stmt->get_result()->fetch_assoc();

// Lấy thông tin sản phẩm
$product_query = "SELECT * FROM sanpham WHERE MaSP = ?";
$stmt = $conn->prepare($product_query);
$stmt->bind_param("i", $product_id);
$stmt->execute();
            ...
            <div class="col-md-8 col-lg-9 chat-area">
                ...
                <?php if ($seller_id && $product_id): ?>
                   <div class="p-3 border-bottom">
                    <h4><?php echo htmlspecialchars($chat_partner['TenNguoiDung']); ?></h4>
                    <div class="small text-muted"><?php echo htmlspecialchars($product['TenSP']); ?></div>
                </div>
