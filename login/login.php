    ...
    <div class="form-box">
        ...
        <div class="register-container" id="register">
            ...
            <div id="errorMessages" class="alert alert-danger" style="display: none;"></div>
                <div class="input-box">
                    <input type="text" class="input-field" name = "txtName" placeholder="Tên người dùng" required>
                   
                    <i class="bx bx-user"></i>
                </div>
                <div class="input-box">
                    <input type="text" class="input-field" name = "txtUsername" placeholder="Tên đăng nhập" required>
                    <i class="bx bx-user"></i>
                </div>
            <div class="input-box">
                <input type="text" class="input-field" name = "txtEmail" placeholder="Email" required>
                <i class="bx bx-envelope"></i>
            </div>
            <div class="input-box">
                <input type="text" class="input-field" name = "txtDiachi" placeholder="Địa chỉ" required>
                <i class="bx bx-envelope"></i>
            </div>
            <div class="input-box">
                <input type="Date" class="input-field" name = "txtNamsinh" placeholder="Năm sinh" required>
                <i class="bx bx-calendar"></i>
            </div>
            <div class="input-box">
                <input type="text" class="input-field" name = "txtPhone" placeholder="Số điện thoại" required>
                <i class="bx bx-phone"></i>
            </div>
            <div class="input-box">
                <input type="password" class="input-field" name = "txtPassword" placeholder="Mật khẩu" required>
                <i class="bx bx-lock-alt"></i>
            </div>
            <div class="input-box">
...
<script>
    $(document).ready(function() {
        ...
        var dob = $('#txtNamsinh').val();
        var phone = $('#txtPhone').val().trim();
        var phoneRegex = /^(03|07|08|09)\d{8}$/;
        if (username === '') {
        ...
        if (name === '') {
            ...
            errors.push('Tên người dùng phải viết hoa chữ cái đầu và không chứa ký tự đặc biệt');
        }
        if (email === '') {
            errors.push('Email không được để trống');
        }
        if (dob === '') {
            errors.push('Ngày tháng năm sinh không được để trống');
        } 
        if (!phoneRegex.test(phone)) {
            errors.push('Số điện thoại phải có 10 chữ số và bắt đầu bằng 03, 07, 08 hoặc 09');
        }else {
            var currentDate = new Date();
            var inputDate = new Date(dob);
            if (inputDate > currentDate) {
                errors.push('Ngày tháng năm sinh không được lớn hơn ngày hiện tại');
            }
        }
        ...
        if (errors.length > 0) {
            ...
            errorHtml += '</ul>';
            $('#registerMessages').html(errorHtml).addClass('alert-danger').show();
        } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        $('#registerMessages').removeClass('alert-danger').addClass('alert-success').text(response.message).show();
                        ...
                        $('#registerMessages').removeClass('alert-success').addClass('alert-danger').html(errorMessage).show();
                    }
                },
                error: function() {
                    $('#registerMessages').removeClass('alert-success').addClass('alert-danger').text('Có lỗi xảy ra. Vui lòng thử lại.').show();
                }
            });
        }
    });
