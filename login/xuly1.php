...
function validateName($name) {
    // Kiểm tra chữ cái đầu viết hoa, cho phép khoảng trắng và dấu, không cho phép ký tự đặc biệt và số
    return preg_match('/^([A-ZÀÁÂÃÈÉÊÌÍÒÓỒỐÔÕÙỪỨÚĂĐĨŨƠƯẠẮẰẶ-ỹ][a-zàáâãèéêìíòóôồốõùưừứúăđĩũơưạắặ-ỹ]*)(\s[A-ZÀÁÂÃÈÉÊÌÍÒÓỒỐÔÕÙÚĂĐĨŨƠƯẠẮẰẶ-ỹ][a-zàáâãèéêìíòóồốôõùúăđĩũơưạ-ỹ]*)*$/', $name);
}
function validateUsername($username) {
    // Kiểm tra tên đăng nhập không chứa ký tự đặc biệt và khoảng trắng
    ...
    if (!validateName($tenNguoiDung)) {
        echo "<script>
                    alert('Tên người dùng phải viết hoa kí tự đầu và không được dùng kí tự đặt biệt.');
                    window.location.href = 'login.php';  // Chuyển hướng đến trang login.php
                  </script>";;
        exit;
    }
    ...
    } elseif (!validateUsername($tenDangNhap)) {
        echo "<script>
        alert('Tên đăng nhập chỉ được chứa chữ cái và số, không được chứa ký tự đặc biệt hoặc khoảng trắng.');
        window.location.href = 'login.php';  // Chuyển hướng đến trang login.php
      </script>";;
    }
    if (empty($email)) {
        ...
        echo "<script>
                    alert('Email phải có định dạng xxx@email.com và không chứa khoảng trắng.');
                    window.location.href = 'login.php';  // Chuyển hướng đến trang login.php
                  </script>";;
    } 
    // Kiểm tra mật khẩu
    if (strlen($matKhau) <= 8) {
        echo "<script>
                    alert('Mật khẩu phải lớn hơn 8 chữ số.');
                    window.location.href = 'login.php';  // Chuyển hướng đến trang login.php
                  </script>";;
        exit;
    }
    if (!preg_match('/^(03|07|08|09)\d{8}$/', $soDienThoai)) {
        echo "<script>
                    alert('Số điện thoại phải có 10 chữ số và bắt đầu bằng 03, 07, 08, 09.');
                    window.location.href = 'login.php';  // Chuyển hướng đến trang login.php
                  </script>";;
    }
